...
export interface AlertOptions {
  component?: string; // e.g. scraper, db, server
...
function sanitize(val: any) {
  if (val == null) return '';
  if (typeof val === 'string') return val.slice(0, 1500);
  try { return JSON.stringify(val).slice(0, 1500); } catch { return String(val).slice(0, 1500); }
}

export async function sendAlert(message: string, opts: AlertOptions = {}) {
  ...
  try {
    ...
    console.error('[ALERT SEND FAILED]', (e as any)?.message);
  }
}
